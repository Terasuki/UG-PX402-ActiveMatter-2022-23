...
def main(run_id, polar, folder, parameters):
    ...
    def simulate(rods, motors, n, m, L, timesteps):
        ...
        anglecount = np.zeros(timesteps)
        for _ in range(timesteps):
            total_polar = np.zeros(2)
            rod_count = 0
            ...
            for rod in rods.values():
                total_polar += np.array(rod.get_polarisation())
                rod_count += 1
            angle = total_polar / rod_count
            anglecount[_] = angle[0]

        return rods, motors, Forcecount, anglecount
    polar_over_time = np.zeros((finalTime, 5))
    for run in range(5):
        ...
        for timestep in range(finalTime):
            rods, motors, _, polar_over_time[timestep] = simulate(rods, motors, n_cols, n_rows, L, recordTime)

    polarity_total = np.mean(polar_over_time, axis=1)
    with open(f'{folder}/polar-all.dat', 'a') as f:
        for row in range(finalTime):
            f.write(f'{polarity_total[row]}\n')
...
if __name__=='__main__':
    ...
    start = time.time()
    for i, d_p in enumerate(np.linspace(0, pol, n_runs)):
        main(i, d_p, folder, parameters)
        middle = time.time() - start
        print(f'Run: {i+1}/{n_runs} - took {middle} seconds')
    end = time.time()
    elapsed = end-start
